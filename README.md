...
The spec called for an AWS Lambda triggered by DynamoDB Streams as the outbox drainer. Enabling Streams requires `dynamodb:UpdateTable` — blocked by the same IAM boundary as (a). The architecture is **unchanged**: `drainOutbox()` in `lib/server/engine.ts` is the unit of work regardless of how it is triggered. Today it runs as a Vercel Cron job (`vercel.json`, every minute) plus a fire-and-forget call after each sale. Replacing the trigger with a Lambda behind Streams or SQS would be a deployment change, not a code change.
